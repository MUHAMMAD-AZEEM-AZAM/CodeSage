...
async def review_pull_request(request: PRReviewRequest):
    """Review a GitHub pull request using Blackbox.ai"""
    if not BLACKBOX_API_KEY:
        raise HTTPException(status_code=500, detail="Blackbox.ai API not configured")
    ...
    async with httpx.AsyncClient() as client:
        ...
        # Send to Blackbox.ai (simulated response for demo)
        # In production, you would make an actual API call to Blackbox.ai
        review_result = await simulate_blackbox_review(review_data)
        review_result['pr_number'] = pr_number
        return PRReviewResponse(**review_result)

async def simulate_blackbox_review(review_data: dict) -> dict:
    """Simulate Blackbox.ai review (replace with actual API call)"""
    # This is a simulation - replace with actual Blackbox.ai API call
    diff_lines = review_data["diff"].split('\n')
    issues = []
    suggestions = []
    ...
    for i, line in enumerate(diff_lines):
        if line.startswith('+') and 'TODO' in line:
            issues.append({
                ...
                "line": i + 1,
                "suggestion": "Remove TODO or create an issue"
            })
        
        if line.startswith('+') and 'console.log' in line:
            issues.append({
                "type": "debug",
                "severity": "info",
    ...
    # Generate suggestions based on PR size
    if review_data["additions"] > 100:
        suggestions.append("Consider breaking this PR into smaller, more focused changes")
    
    if review_data["files_changed"] > 10:
        suggestions.append("This PR touches many files. Consider if this could be split into multiple PRs")
    
    return {
        "summary": f"Review completed for PR #{review_data.get('pr_number', 'N/A')}. Found {len(issues)} issues and {len(suggestions)} suggestions.",
        "issues": issues,
